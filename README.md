# 📚 Dicoding Submission Portfolio

<div align="center">

![Dicoding](https://img.shields.io/badge/Dicoding-Academy-2d79c7?style=for-the-badge&logo=data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAyNCAyNCI+PHBhdGggZmlsbD0id2hpdGUiIGQ9Ik0xMiAyTDIgN2wxMCA1IDEwLTV6Ii8+PHBhdGggZmlsbD0id2hpdGUiIGQ9Ik0yIDE3bDEwIDUgMTAtNXYtNmwtMTAgNS0xMC01eiIvPjwvc3ZnPg==)
![Status](https://img.shields.io/badge/Status-In%20Progress-yellow?style=for-the-badge)
![Last Updated](https://img.shields.io/badge/Updated-January%202026-brightgreen?style=for-the-badge)

**Kumpulan semua submission yang pernah saya kerjakan di Dicoding Academy, lengkap dengan feedback dari reviewer.**

[Tentang](#-tentang) •
[Daftar Submission](#-daftar-submission) •
[Struktur Proyek](#-struktur-proyek) •
[Cara Penggunaan](#-cara-penggunaan)

</div>

---

## 🎯 Tentang

Repository ini merupakan dokumentasi portofolio dari perjalanan belajar saya di **[Dicoding Academy](https://www.dicoding.com/)**. Di sini, saya mengumpulkan semua submission dari berbagai kelas yang pernah saya ikuti, beserta:

- ✅ **Source code lengkap** dari setiap submission
- 📝 **Feedback reviewer** yang sangat berharga untuk pembelajaran
- 📸 **Screenshot hasil** (jika relevan)
- 📋 **Catatan pribadi** tentang apa yang dipelajari

### Tujuan Repository Ini

1. **Dokumentasi Perjalanan Belajar** - Menyimpan jejak pembelajaran dan perkembangan skill
2. **Portfolio** - Menunjukkan hasil karya kepada calon employer atau kolaborator
3. **Referensi** - Sebagai bahan referensi untuk proyek-proyek mendatang
4. **Berbagi Pengetahuan** - Membantu sesama learner yang membutuhkan inspirasi

---

## 📋 Daftar Submission

### 🤖 Machine Learning

| No | Kelas | Submission | Status | Rating |
|----|-------|------------|--------|--------|
| 1 | Belajar Machine Learning untuk Pemula | *Coming Soon* | ⏳ | - |
| 2 | Belajar Fundamental Deep Learning | *Coming Soon* | ⏳ | - |
| 3 | Membangun Sistem Machine Learning | *Coming Soon* | ⏳ | - |
| 4 | Machine Learning Terapan | *Coming Soon* | ⏳ | - |

### 📊 Data Science

| No | Kelas | Submission | Status | Rating |
|----|-------|------------|--------|--------|
| 1 | Belajar Fundamental Pemrosesan Data | *Coming Soon* | ⏳ | - |
| 2 | Belajar Fundamental Analisis Data | *Coming Soon* | ⏳ | - |
| 3 | Belajar Penerapan Data Science | *Coming Soon* | ⏳ | - |

> 📌 **Catatan:** Tabel di atas akan diperbarui seiring dengan penambahan submission baru.

---

## 📁 Struktur Proyek

```
📦 Submission_Dicoding/
├── 📄 README.md
├── 📂 Machine-Learning/
│   ├── 📂 01-Belajar Machine Learning untuk Pemula/
│   │   ├── 📂 submission/
│   │   ├── 📄 README.md (penjelasan & screenshot)
│   │   └── 📄 FEEDBACK.md (feedback dari reviewer)
│   └── ...
└── 📂 Data-Science/
    └── ...
```

### Format Setiap Submission

Setiap folder submission akan mengandung:

```
📂 [Nama-Kelas]/
├── 📂 submission/          # Source code submission
├── 📂 screenshots/         # Screenshot hasil (opsional)
├── 📄 README.md           # Penjelasan tentang proyek
├── 📄 FEEDBACK.md         # Feedback lengkap dari reviewer
└── 📄 NOTES.md            # Catatan pribadi (opsional)
```

---

## 🚀 Cara Penggunaan

### Clone Repository

```bash
git clone https://github.com/[username]/Submission_Dicoding.git
cd Submission_Dicoding
```

### Navigasi

Setiap submission memiliki README sendiri yang menjelaskan:
- Cara menjalankan proyek
- Dependencies yang dibutuhkan
- Fitur-fitur yang ada
- Kriteria yang harus dipenuhi

---

## 📊 Statistik

<div align="center">

| Kategori | Total Submission | Completed |
|----------|-----------------|-----------|
| Machine Learning | 0 | 0 |
| Data Science | 0 | 0 |
| **Total** | **0** | **0** |

</div>

---

## 📝 Template Feedback

Berikut adalah template yang digunakan untuk menyimpan feedback reviewer:

```markdown
# Feedback Reviewer

## 📅 Tanggal Submit
[Tanggal submission]

## ⭐ Rating
[Bintang yang didapat]

## 💬 Komentar Umum
[Komentar umum dari reviewer]

## ✅ Yang Sudah Baik
- [Point 1]
- [Point 2]

## 🔧 Saran Perbaikan
- [Saran 1]
- [Saran 2]

## 📌 Catatan Tambahan
[Catatan lain yang perlu diingat]
```

---

## 🤝 Kontribusi

Jika Anda memiliki saran atau menemukan kesalahan, silakan:
1. Fork repository ini
2. Buat branch baru
3. Submit Pull Request

---

## 📬 Kontak

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/zidan-mubarak/)
[![Dicoding](https://img.shields.io/badge/Dicoding-Profile-2d79c7?style=for-the-badge)](https://www.dicoding.com/users/zidanmubarak/)

</div>

---

<div align="center">

**Made with ❤️ for Learning**

*"Setiap submission adalah langkah maju dalam perjalanan belajar"*

</div>
